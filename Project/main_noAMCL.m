...
prm_cfg.simplifyEps = 0.40; % Removes unnecessary breadcrumbs in straight hallways
    ...
    try
        ...
        elseif isa(rawMap,'binaryOccupancyMap')
            ...
            try 
                ...
                locMap.LocalOriginInWorld = rawMap.LocalOriginInWorld;
            end
        else
...
%% Initial Map-to-Odometry Transform
% Set up persistent transforms to map raw Odometry frame to the True Map frame.
...
try
    while toc(mission_start) < max_mission_time
        ...
        % ----------------------------------------------------
        % 1. LOCALIZATION (Odometry Tracking)
        % ----------------------------------------------------
        % Read live Odometry
        odom_pose = g_pose(:)';
        ...
        % Calculate raw robot pose from initial map offset + live odometry 
        T_M_R = T_M_O * T_O_R;
            ...
            case 'NAV_TO_B'
                ...
                if dist_to_wp <= active_tol
                    ...
                    if waypoint_idx > num_waypoints
                        ...
                        % Let the loop smoothly fall through to calculate PID for the new waypoint
                    end
                end
