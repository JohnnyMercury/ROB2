...
max_amcl_jump = 0.3;         % Max allowed distance (m) AMCL can shift the robot during normal driving
max_amcl_jump_warmup = 1.5;  % Allow larger snaps during the initial 5-second warmup
...
prm_cfg.connectionDistance = 2.5;
...
found_blue = false;  
target_color_focus =''; 
search_waypoints = [20.291, 9.693; 20.241, 10.896; 20.187, 12.211; 20.156, 13.245; 20.119, 14.906; 20.004, 16.911; 20.004, 18.343;]; % Area B 
...
sm.MaxLikelihoodDistance = 0.38; % INCREASED: Widens the snap distance so it aligns to walls even when drifting heavily
...
% ADDED: Start in a warmup state to let AMCL process scans for 5 seconds
state = 'INIT_AMCL';
controller_state = [];  
avoid_state = [];      
...
try
    while toc(mission_start) < max_mission_time
        ...
        if g_scan_received && ~isempty(g_scan)
            ...
            if isUpdated
                % --- TELEPORTATION LIMITER ---
                % Calculate how far AMCL wants to move the robot from our current belief
                jump_dist = hypot(estimatedPose(1) - pose(1), estimatedPose(2) - pose(2));

                                active_jump_limit = max_amcl_jump;
                if strcmp(state, 'INIT_AMCL')
                    active_jump_limit = max_amcl_jump_warmup; % Allow larger snaps during the initial warmup
                end
                
                if jump_dist <= active_jump_limit
                    % Normal update - calculate the new Map -> Odom transform
                    ...
                    % Reject update if it tried to teleport through a wall
                    fprintf('[AMCL] REJECTED TELEPORT: Ignored invalid jump of %.2f meters (Limit: %.2fm)\n', jump_dist, active_jump_limit);
                    
                    % --- FIX HALLUCINATION ---
                    % Force AMCL to drop its hallucinated particles and respawn them 
                    % around our current, trusted odometry-based pose.
                    fprintf('[AMCL] Re-seeding particle cloud at trusted location to clear hallucination.\n');
                    release(amcl); % REQUIRED: Unlock the AMCL object before modifying non-tunable properties
        ...
        switch state
            case 'INIT_AMCL'
                % Force robot to sit still for 5 seconds while AMCL reads incoming Lidar
                v_cmd = 0; 
                w_cmd = 0;
                
                if t_elapsed >= 5.0
                    fprintf('[STATE] AMCL 5-second warmup complete. Switching to NAV_TO_B.\n');
                    release(amcl);  % REQUIRED: Unlock the AMCL object before modifying non-tunable properties
                    amcl.UpdateThresholds = [0.10, 0.10, 0.10]; % Restore normal thresholds to save CPU during driving
                    state = 'NAV_TO_B';
                end
            ...
            case 'SEARCH_SPIN'
                v_cmd = 0.0; w_cmd = 0.25; % Slow rotation
                ...
                if g_image_received && ~isempty(g_image)
                    ...
                    if ~isempty(centersO) && ~found_orange
                        fprintf('[STATE] Orange target detected! Bracking and switching to ALIGN_AND_PICTURE.\n');
                        target_color_focus = 'Orange';
                    ...
                    elseif ~isempty(centersB) && ~found_blue
                        fprintf('[STATE] Blue target detected! Bracking and switching to ALIGN_AND_PICTURE.\n');
                        target_color_focus = 'Blue';
                        vel_msg.linear.x = 0; vel_msg.angular.z = 0; send(vel_pub, vel_msg);
                        pause(0.5);
                        state = 'ALIGN_AND_PICTURE';
        ...
        if enable_visualization && mod(loop_count, viz_update_stride) == 0
            ...
            if (strcmp(state, 'SEARCH_DRIVE') || strcmp(state, 'SEARCH_SPIN') || strcmp(state, 'ALIGN_AND_PICTURE')) && g_image_received && ~isempty(g_image)
                figure(3);
                image_rgb_viz = imrotate(g_image, 180);
                [cO, rO, cB, rB] = detectCircles(image_rgb_viz);
                
                imshow(image_rgb_viz);
                hold on;
                if ~isempty(cB)
                    viscircles(cB(1,:), rB(1), 'EdgeColor', 'b'); 
                end
                if ~isempty(cO)
                    viscircles(cO(1,:), rO(1), 'EdgeColor', 'r'); 
                end
                hold off;
            end
...
function [centersO, radiiO, centersB, radiiB] = detectCircles(rgb)
    ...
    maskO = (hsv(:,:,1) >= 0.05 & hsv(:,:,1) <= 0.15) & hsv(:,:,2) >= 0.5 & hsv(:,:,3) >= 0.2; % Orange 
    maskB = (hsv(:,:,1) >= 0.55 & hsv(:,:,1) <= 0.65) & hsv(:,:,2) >= 0.5 & hsv(:,:,3) >= 0.2; % blue
    se = strel('disk', 5);
    % Clean up the masks
