...
%% Define subscribers (without callbacks - read LatestMessage instead)
...
try
    ...
    while true
        ...
        heading_desired = atan(position_desired(2) / position_desired(1));
        ...
        %% Visualise distance to desired position
        % Compute Euclidean distance to goal and append to history.
        distance_to_target = norm(position_desired - position);
        ...
        y_error = y_desired - y_actual;
        
        distance_error = sqrt(x_error^2 + y_error^2);
        
        heading_desired = atan2(y_error, x_error);
        ...
        % Compute Euclidean distance to goal (already computed above)
        % distance_to_target is positive when far from goal
        %position_error_mag = distance_to_target;  % Alias for clarity in PI controller
       
        % Accumulate integral of distance error over time
        % Prevents steady-state error from proportional-only control
